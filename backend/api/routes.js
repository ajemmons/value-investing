...
router.post('/portfolio', async (req, res) => {
  try {
    ...
    // Serve a PRECOMPUTED full-universe portfolio when available: the default
    // (no custom list) full-universe build is too heavy to run live on a small
    // host, so we return the committed snapshot instantly. `fresh:true` forces a
    // live recompute; a custom universe always computes live.
    ...
    res.json(buildPortfolioResponse(results, { riskLevel, size, perSector, universeMeta }));
  } catch (err) {
